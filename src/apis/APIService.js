...
class APIService {
  constructor() {
    ...
    this.ConfigHost = process.env.DB_HOST_v1;
  ...
  execute = async (request) => {
    try {
      ...
      if (responseJson.message === "Invalid Token") {
        ...
        window.location.href = `${url.replace(path, "")}/login`;
      }
  ...
  _getListOrder = async ({ page, limit }) => {
    try {
      var params = this.getDefaultParams();
      params = { ...params, ...{ page, limit } };
      let request = this.get("/invoices", params);
      let data = await this.execute(request);
      return data;
    } catch (error) {
      throw error;
  ...
  _getListProduct = async ({ page, limit }) => {
    try {
      var params = this.getDefaultParams();
      params = { ...params, ...{ page, limit } };
      let request = this.get(`/products`, params);
      let data = await this.execute(request);
      return data;
    } catch (error) {
      throw error;
  ...
  _getListLevelUser = async ({ user_id, level, page, limit }) => {
    try {
      var params = this.getDefaultParams();
      ...
      let request = this.get(`/users/get-level`, params);
      let data = await this.execute(request);
      return data;
    } catch (error) {
      throw error;
