...
import {
  _reducerFetchExportWarehouseCreate,
  _reducerFetchExportWarehouseItem,
...
export default function CreateExportWarehouses() {
  ...
  const _fetchItem = async (id_import_warehouse) => {
    try {
      const data = await dispatch(
        _reducerFetchExportWarehouseItem(id_import_warehouse)
      );
      ...
      form.setFieldsValue({
        ...
        export_for_warehouse: currentData.export_for_warehouse,
  ...
  const onFinish = async (values) => {
    if (Number(id) === 0 && type === "create") {
      ...
      values.product_list.map((item) => {
        item.supplier_name = values.export_for_warehouse;
      });
      const dataAdd = {
        notes: values.notes,
        title: `${values.export_for_warehouse}`,
        created_date: timeDob,
        export_for_warehouse: values.export_for_warehouse,
        ...
        supplier_name: values.export_for_warehouse,
      };
      const data = await dispatch(_reducerFetchExportWarehouseCreate(dataAdd));
      const currentData = unwrapResult(data);
      if (currentData.code === "Fail") {
      ...
      navigate.push("/warehouses/export");
    } else {
      setLoadingSubmit(true);
      var dateParts = currentDob.split("/");
      let timeDob = new Date(+dateParts[2], dateParts[1] - 1, +dateParts[0]);
      ...
      const data = await APIService._putExportWarehouse(id, dataUpdate);
      if (data.code === "Fail") {
        setLoadingSubmit(false);
      ...
      <Skeleton active paragraph={{ rows: 12 }} loading={loading}>
        <Form form={form} onFinish={onFinish} autoComplete="off" {...layout}>
          <PageHeader
            ...
            title="PHIẾU XUẤT HÀNG / TRẢ HÀNG "
          ...
          >
            <Row gutter={[16, 0]}>
              <Col span={12}>
                <Form.Item
                  hasFeedback
                  name="export_for_warehouse"
                  label="Kho xuất"
                  rules={[
                    {
                      required: true,
                      message: "Vui lòng điền tên Kho xuất.",
                    },
                  ]}
                >
                  <ISelect
