...
import {
  _reducerFetchListImportWarehouse,
  _reducerFetchCountImportWarehouse,
  _reducerFetchDeleteImportWarehouse,
} from "../../reducers/Warehouses/reducer";
// import CreateStationPage from "./CreateStationPage";

const formatDay = "#hhh#:#mm#:#ss# #DD#/#MM#/#YYYY#";

export default function ListImportWarehouses() {
  ...
  const _fetchTotalCount = async () => {
    try {
      const data = await dispatch(_reducerFetchCountImportWarehouse());
  ...
  const _fetchAPIRemove = async (idStations) => {
    try {
      const data = await dispatch(
        _reducerFetchDeleteImportWarehouse(idStations)
      );
      const currentData = unwrapResult(data);
      if (currentData.code === "Fail") {
        message.error(currentData.message);
  ...
  const columns = [
    ...
    {
      title: "Kho nhập",
      dataIndex: "import_for_warehouse",
      key: "import_for_warehouse",
    },
    {
      ...
      render: (obj) => (
        <Space size={12}>
          <Tooltip title="Chi tiết">
            <Link to={`/warehouses/import/${obj.id}`}>
              <InfoCircleOutlined style={{ color: blue[5] }} />
            </Link>
          </Tooltip>
          <Tooltip title="Chỉnh sửa">
            <Link to={`/warehouses/import/edit/${obj.id}`}>
              <EditOutlined />
            </Link>
          </Tooltip>
          <Tooltip title="Xóa">
  ...
  const _fetchAPIList = async (filterTable) => {
    try {
      const data = await dispatch(
        _reducerFetchListImportWarehouse(filterTable)
      );
      const currentData = unwrapResult(data);
      if (currentData.code === "Fail") {
        message.error(currentData.message);
    ...
    <div>
      <Row gutter={[0, 24]}>
        ...
        <Col span={12} style={{ textAlign: "right" }}>
          <Link to={`/warehouses/import/create/0`}>
